...
**Still not verified:** real microphone transcription, model-driven workout planning/correction/progress end-to-end, physical-iPhone audio/background behaviour, and recovery through two actual browser profiles/devices. Independent authenticated API clients have verified saved-data recovery, but are not a substitute for these checks. The physical test below is the remaining user action, not another key-setup task.
