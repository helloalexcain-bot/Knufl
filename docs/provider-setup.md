...
- Migrations `202609050001` through `202609050007`, plus `202609060001` (persisted training context), are applied and recorded in Supabase migration history.
...
- Following Google setup, all 13 deployed integration groups passed again, including privileged cross-account mutation checks; both disposable accounts and their records were removed. Signed-in onboarding was inspected at 390×844 and 1280×900, with no captured browser warnings/errors. This was followed by the separate live OpenAI control checks below.
...
### Observed live OpenAI control results — 5 September 2026

- Private deployment version 4, runtime revision 3, succeeded. The configured model is `gpt-realtime-2.1`. Real browser WebRTC connected twice and received OpenAI's `session.created` event. Neither test requested microphone access or generated a reply.
- Both real calls were independently terminated by Supabase Cron, **not a client timer or close request**. At deadlines 20:41:21.711 UTC and 20:43:59.618 UTC, OpenAI returned HTTP 200; ledger closure was respectively **1.116 seconds** and **0.998 seconds** later. Browser data channels closed shortly afterwards. Each outbox recorded one acknowledged attempt.
- On the second active call, another call from the same account was denied with 429, account B's authenticated Worker close request for A's call returned 404, and a subsequent call after the daily allowance was consumed returned 429 / `daily_budget_exhausted`. The 59-minute prior usage was a **disposable fixture** to leave a one-minute real call; no deployment limits were changed.
- All disposable users/data from both call tests and the 13-group deployed isolation/recovery suite were removed. The private owner gate stayed unchanged; all five approved artwork hashes matched, and public GitHub Pages remained available.
- Current checks: **97 unit/contract tests**, **96 local PostgreSQL assertions**, **96 hosted pgTAP assertions**, lint, TypeScript, Cloudflare build and `/Knufl/` build passed. pgTAP transport responses and Node provider tests are simulated; the two WebRTC sessions and provider acknowledgements above are not.
- The voice-enabled character screen survived refresh and was inspected at 390×844 and 1280×900 with no captured browser warnings/errors. The preview automation credential was rotated; the previous credential returned 401 and its replacement returned 200.

The above results are historical. See [6 September voice-feedback verification](voice-feedback-verification.md) for the subsequent actual-model tests and articulated character study. Physical-iPhone audio/background behaviour and recovery through two actual browser profiles/devices still require the short physical test below. Independent authenticated API clients are not a substitute for those checks. No additional key setup is required.
...
- `node scripts/test-live-realtime.mjs`: **opt-in billable** browser control test. Run only with the owner's API-spend authorisation. Supply a JSON object containing `anonKey`, `serviceRoleKey`, and `siteToken` through echo-disabled stdin, never arguments or chat. Open its printed localhost URL and click **Connect once**. It creates two disposable users, gives A a 59-minute closed usage fixture, verifies real WebRTC/concurrency/foreign-close rejection, then observes independent ledger closure and daily-budget denial. Confirm the browser's remote CLOSED event and the private outbox's 200 acknowledgement before **Clean up test**. No microphone or model reply is requested. The localhost server has a ten-minute cleanup watchdog; global runtime limits are untouched. It deliberately is not part of `npm test`.
- `node scripts/test-live-conversation.mjs`: **opt-in billable** actual-model conversation checks. Uses the same hidden-stdin credential object, a disposable account, the shipped Realtime client and real deployed tools. Typed input is sent to OpenAI, not the offline phrase parser; output is real WebRTC audio. Run the bench sequence, then reconnect/superset sequence, then clean up. Maximum three calls and an eight-minute cleanup watchdog. It waits for a final spoken reply, not just the end of a tool preamble. It does not request a microphone or change preview access/limits.
  Add `--focused` for the four-step ambiguity/switch/set/progress check with an explicitly seeded superset plan. Harness-only diagnostics print response status/finish metadata, not audio or credentials. A timeout is a failed run even when a tool saved correctly.
...
3. Say **“Start a ninety-second rest.”** Reload: same end time, not a new ninety seconds. The three bench sets from step 2 should be six, six and eight, all at sixty kilograms total; correction must retain the first set's ID and audit history.
4. Ask **“What is my bench-press progress today?”** The answer must reflect those actual saved sets, with a stated comparison basis—not claim improvement from the plan. Interrupt a reply; the mouth and gestures should settle smoothly. Test mute and push-to-talk. A spoken acknowledgement without a saved receipt is not success.
5. On another device/isolated browser profile, pass the **same owner gate** and use the **same Google account**. Verify the chosen companion name, corrected sets and saved timer. Returning focus/online on device one must refresh cloud facts. Then use app account B: A's data must be absent. Automated Worker attacks complement this UI check.
